...
impl<T: InvokeUiCM> IpcTaskRunner<T> {
    async fn run(&mut self) {
        ...
        let mut file_timer =
            crate::veradesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
            ...
            tokio::select! {
                            ...
                            match data {
                                ...
                                Data::FS(mut fs) => {
                                    ...
                                    if !self.read_jobs.is_empty() {
                                        file_timer = crate::veradesk_interval(time::interval(MILLI5));
                                    }
                ...
                _ = file_timer.tick() => {
                    if !self.read_jobs.is_empty() {
                        ...
                        file_timer = crate::veradesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn read_all_files_success() {
        ...
        rt.block_on(async {
            ...
            let dir = std::env::temp_dir().join("veradesk_read_all_test");
    ...
    fn read_dir_reports_success_and_error() {
        ...
        rt.block_on(async {
            let (tx, mut rx) = unbounded_channel();
            let dir = std::env::temp_dir().join("veradesk_read_dir_test");
            let _ = fs::remove_dir_all(&dir);
            fs::create_dir_all(&dir).unwrap();
            ...
            match rx.recv().await.unwrap() {
                Data::RawMessage(bytes) => {
                    ...
                    assert!(msg
                        ...
                        .dir()
                        .path
                        .contains("veradesk_read_dir_test"));
                }
                _ => panic!("unexpected data"),
            }
            let _ = fs::remove_dir_all(&dir);
    ...
    fn test_symlink_creation_works() {
        let base_dir = std::env::temp_dir().join("veradesk_symlink_test");
