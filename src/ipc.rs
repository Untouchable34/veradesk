    ...
    match data {
        ...
        Data::Close => {
            ...
            if EXIT_RECV_CLOSE.load(Ordering::SeqCst) {
                ...
                if crate::is_main() {
                    // below part is for main windows can be reopen during veradesk installation and installing service from UI
                    ...
                    #[cfg(target_os = "linux")]
                    {
                        hbb_common::sleep((crate::platform::SERVICE_INTERVAL * 2) as f32 / 1000.0)
                            .await;
                        // https://github.com/rustdesk/veradesk/discussions/9254
