...
const IDD_IMPL_VERADESK: &str = "veradesk_idd";
...
pub fn get_cur_device_string() -> &'static str {
    match IDD_IMPL {
        IDD_IMPL_VERADESK => VERADESK_IDD_DEVICE_STRING,
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_VERADESK => veradesk_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_VERADESK => {
            let virtual_displays = veradesk_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "veradesk_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_VERADESK => veradesk_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_VERADESK => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                veradesk_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            veradesk_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_VERADESK => veradesk_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_VERADESK => veradesk_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_VERADESK => veradesk_idd::reset_all(),
        IDD_IMPL_AMYUNI => amyuni_idd::reset_all(),
        _ => bail!("Unsupported virtual display implementation."),
    }
}

pub mod veradesk_idd {
    use super::windows;
    ...
    fn get_device_names() -> Vec<String> {
        windows::get_device_names(Some(super::VERADESK_IDD_DEVICE_STRING))
    }
...
pub mod amyuni_idd {
    ...
    // 2. VeraDesk may crash and restart, but the virtual displays are kept.
    //
    ...
    //             if there're no virtual displays managed by VeraDesk.
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
        ...
        //
        // 1. VeraDesk plug in 2 virtual displays. (VeraDesk)
        // 2. Other process plug out all virtual displays. (User manually)
        // 3. Other process plug in 1 virtual display. (User manually)
        // 4. VeraDesk plug out all virtual displays in this call. (VeraDesk disconnect)
        //
        // This is not a normal scenario, VeraDesk will plug out virtual display unexpectedly.
