...
pub(super) fn embedded_shortcut_commands(bytes: Vec<u8>, filename: &str, name: &str) -> String {
    ...
    format!(
        "> \"{encoded_path}\" echo {encoded}\r\n\
         certutil -f -decode \"{encoded_path}\" \"%VERADESK_OUTPUT_DIR%\\{filename}\" > nul || exit /b {BATCH_SHORTCUT_DECODE_FAILURE_EXIT_CODE}"
    )
}
...
mod tests {
    ...
    fn install_values_enforce_command_safety() {
        ...
        assert!(validate_install_value(r"C:\Program Files (x86)\VeraDesk").is_ok());
        assert!(validate_install_value(r"C:\Users\R&D\VeraDesk.exe").is_ok());
        assert!(validate_install_value(r"C:\A&^ B\VeraDesk.exe").is_ok());
    ...
    fn nested_commands_escape_while_protected_environment_preserves_carets() {
        assert_eq!(
            escape_nested_cmd_ampersands(r"C:\A&^ B\VeraDesk.exe"),
            r"C:\A^&^^ B\VeraDesk.exe"
        );
