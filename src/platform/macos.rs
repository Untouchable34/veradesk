...
fn get_update_temp_dir() -> PathBuf {
    ...
    Path::new("/tmp").join(format!(".veradeskupdate-{}", euid))
}
...
// remove just one app from all the permissions: tccutil reset All com.veranilsoft.veradesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.veranilsoft.veradesk", &bundleid);
    }
    s = s.replace("veradesk", &crate::get_app_name().to_lowercase());
    s = s.replace("VeraDesk", &crate::get_app_name());
    s
}
...
pub fn write_plists() -> ResultType<()> {
    let daemon_plist_path = format!(
        "/Library/LaunchDaemons/com.veranilsoft.{}_service.plist",
        crate::get_app_name()
    );
    let agent_plist_path = format!(
        "/Library/LaunchAgents/com.veranilsoft.{}_server.plist",
        crate::get_app_name()
    );
    let Some(daemon_plist) = PRIVILEGES_SCRIPTS_DIR.get_file("daemon.plist") else {
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/VeraDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another VeraDesk.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // VeraDesk.app/Contents/MacOS/VeraDesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
...
pub fn update_from_dmg_as_root(dmg_path: &str, expected_version: &str) -> ResultType<()> {
    ...
    let tmp_dir_output = std::process::Command::new("/usr/bin/mktemp")
        .args(&["-d", "/tmp/.veradeskupdate-root-XXXXXX"])
        .output()?;
    ...
    let agent_plist = format!("/Library/LaunchAgents/com.veranilsoft.{}_server.plist", app_name);
    let daemon_plist = format!("/Library/LaunchDaemons/com.veranilsoft.{}_service.plist", app_name);
    ...
    // We cannot directly replace /Applications/VeraDesk.app while it is running,
    // so we spawn a script that waits, kills processes, copies, and restarts.
    let daemon_label = format!("com.veranilsoft.{}_service", app_name);
    let agent_label = format!("com.veranilsoft.{}_server", app_name);
    let script_path = format!("{}/veradesk_update.sh", tmp_dir);
    let script = format!(
        r#"#!/bin/sh
rollback_done=0
bundle_swapped=0
...
restore_old_bundle() {{
    ...
    if [ ! -d "{app_bundle}.bak" ] || [ -L "{app_bundle}.bak" ]; then
        echo "[root-update] CRITICAL: valid application backup is unavailable" >> {tmp_dir}/veradesk_root_update.log
        return 1
    fi
    if [ -e "{app_bundle}" ] || [ -L "{app_bundle}" ]; then
        if [ -e "{app_bundle}.failed-update" ] || [ -L "{app_bundle}.failed-update" ] || \
           ! mv "{app_bundle}" "{app_bundle}.failed-update"; then
            echo "[root-update] CRITICAL: could not vacate failed bundle safely" >> {tmp_dir}/veradesk_root_update.log
            return 1
        fi
    fi
    if ! mv "{app_bundle}.bak" "{app_bundle}"; then
        echo "[root-update] CRITICAL: failed to restore application bundle" >> {tmp_dir}/veradesk_root_update.log
        if [ ! -e "{app_bundle}" ] && [ ! -L "{app_bundle}" ]; then
...
rollback_transaction() {{
    ...
    touch /var/root/.veradeskupdate_failed || restore_failed=1
    ...
    if [ "$restore_failed" -ne 0 ]; then
        echo "[root-update] CRITICAL: rollback restoration failed" >> {tmp_dir}/veradesk_root_update.log
    else
        echo "[root-update] Rollback daemon and agents verified healthy" >> {tmp_dir}/veradesk_root_update.log
    fi
}}
...
if ! capture_agent_snapshot; then
    echo "[root-update] old LaunchAgent readiness check failed before shutdown" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
capture_stopping_agent_pids
if ! stop_daemon; then
    echo "[root-update] daemon did not stop before bundle swap" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
if ! stop_agents; then
    echo "[root-update] old LaunchAgent did not stop before bundle swap" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
...
if ! stop_user_bundle_processes; then
    echo "[root-update] VeraDesk GUI process did not stop before bundle swap" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
staged_bundle="{tmp_dir}/staged.app"
if [ -e "$staged_bundle" ] || [ -L "$staged_bundle" ]; then
    echo "[root-update] staged bundle path already exists, aborting" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
if ! ditto {src_app} "$staged_bundle" 2>/dev/null; then
    echo "[root-update] ditto failed, aborting update" >> {tmp_dir}/veradesk_root_update.log
    rm -rf "$staged_bundle"
    exit 1
fi
...
if [ ! -d "$staged_bundle/Contents/MacOS" ] || \
   ...
   [ ! -f "$staged_bundle/Contents/Info.plist" ]; then
    echo "[root-update] staged bundle validation failed, aborting" >> {tmp_dir}/veradesk_root_update.log
    rm -rf "$staged_bundle"
    exit 1
fi
if ! mv {app_bundle} {app_bundle}.bak; then
    echo "[root-update] backup mv failed, aborting" >> {tmp_dir}/veradesk_root_update.log
    rm -rf "$staged_bundle"
    exit 1
fi
bundle_swapped=1
if ! mv "$staged_bundle" {app_bundle}; then
    echo "[root-update] replacement mv failed, restoring backup" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
...
if ! chown -R root:wheel {app_bundle} || ! chmod -R go-w {app_bundle}; then
    echo "[root-update] chown failed, restoring backup" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
...
if ! chown root:wheel {app_bundle} || \
   ...
   ! chmod 755 {app_bundle}/Contents/MacOS/{app_name}; then
    echo "[root-update] hardening failed, restoring backup" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
...
if ! write_new_plists; then
    echo "[root-update] CRITICAL: new binary failed to write plists" >> {tmp_dir}/veradesk_root_update.log
    cat "{tmp_dir}/write-plists.log" >> {tmp_dir}/veradesk_root_update.log 2>/dev/null || true
    exit 1
fi
echo "[root-update] Plist definitions written by new binary" >> {tmp_dir}/veradesk_root_update.log
...
if ! launchctl load -w {daemon_plist} 2>/dev/null && \
   ! launchctl bootstrap system {daemon_plist} 2>/dev/null; then
    echo "[root-update] CRITICAL: daemon reload failed, restoring backup" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
if ! daemon_ready; then
    echo "[root-update] CRITICAL: daemon failed readiness check, restoring" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
...
if ! bootstrap_agents || ! agent_ready; then
    echo "[root-update] CRITICAL: agent bootstrap failed, rolling back" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
...
if ! daemon_snapshot_stable || ! agent_snapshot_stable; then
    echo "[root-update] CRITICAL: daemon or agent stopped before commit, restoring" >> {tmp_dir}/veradesk_root_update.log
    exit 1
fi
...
if ! rm -rf "{app_bundle}.bak"; then
    echo "[root-update] WARNING: committed update but could not remove backup" >> {tmp_dir}/veradesk_root_update.log
fi
for gui_uid in $gui_uids; do
    launchctl asuser "$gui_uid" open -a "{app_bundle}" || true
done
echo "[root-update] Done!" >> {tmp_dir}/veradesk_root_update.log
rm -rf {tmp_dir}
"#,
        app_name = app_name,
...
fn extract_dmg_inner(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_output = Command::new("/usr/bin/mktemp")
        .args(["-d", "/tmp/.veradeskmount-XXXXXX"])
        .output()?;
