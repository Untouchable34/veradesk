// https://github.com/rustdesk/veradesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteVeraDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes VeraDesk Test certificates and wrong key stores
//
...
BOOL DeleteVeraDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteVeraDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteVeraDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteVeraDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteVeraDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
