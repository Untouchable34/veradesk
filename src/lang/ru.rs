...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("connecting_status", "Подключение к сети VeraDesk..."),
        ...
        ("install_tip", "В некоторых случаях из-за UAC VeraDesk может работать неправильно на удалённом узле. Чтобы избежать возможных проблем с UAC, нажмите кнопку ниже для установки VeraDesk в системе."),
        ("Click to upgrade", "Нажмите, чтобы обновить"),
        ("Configure", "Настроить"),
        ("config_acc", "Чтобы удалённо управлять своим рабочим столом, вы должны предоставить VeraDesk права \"доступа\""),
        ("config_screen", "Для удалённого доступа к рабочему столу вы должны предоставить VeraDesk права \"снимок экрана\""),
        ...
        ("android_input_permission_tip1", "Чтобы удалённое устройство могло управлять вашим Android-устройством с помощью мыши или нажатий, необходимо разрешить VeraDesk использовать службу \"Специальные возможности\"."),
        ("android_input_permission_tip2", "Перейдите на соответствующую страницу системных настроек, найдите и войдите в \"Установленные службы\", включите службу \"VeraDesk Input\"."),
        ...
        ("Turned off", "Отключён"),
        ("Language", "Язык"),
        ("Keep VeraDesk background service", "Держать в фоне службу VeraDesk"),
        ...
        ("Show VeraDesk", "Показать VeraDesk"),
        ...
        ("config_input", "Чтобы управлять удалённым рабочим столом с помощью клавиатуры, необходимо предоставить VeraDesk разрешения \"Мониторинг ввода\"."),
        ("config_microphone", "Чтобы разговаривать с удалённой стороной, необходимо предоставить VeraDesk разрешение \"Запись аудио\"."),
        ...
        ("still_click_uac_tip", "По-прежнему требуется, чтобы удалённый пользователь нажал \"OK\" в окне UAC при запуске VeraDesk."),
        ...
        ("verify_veradesk_password_tip", "Подтвердить пароль VeraDesk"),
        ...
        ("scam_text1", "Если вы разговариваете по телефону с кем-то, кого вы НЕ ЗНАЕТЕ и НЕ ДОВЕРЯЕТЕ, и он просит вас использовать VeraDesk и запустить его службу, не продолжайте и немедленно прервите разговор."),
        ...
        ("upgrade_veradesk_server_pro_to_{}_tip", "Обновите VeraDesk Server Pro до версии {} или новее!"),
        ...
        ("selinux_tip", "На вашем устройстве включён SELinux, что может помешать правильной работе VeraDesk на управляемой стороне."),
        ("Change view", "Вид"),
        ...
        ("powered_by_me", "Основано на VeraDesk"),
        ...
        ("allow-only-conn-window-open-tip", "Разрешать подключение только при открытом окне VeraDesk"),
        ...
        ("floating_window_tip", "Помогает поддерживать фоновую службу VeraDesk"),
        ...
        ("upgrade_remote_veradesk_client_to_{}_tip", "Обновите клиент VeraDesk до версии {} или новее на удалённой стороне!"),
        ...
        ("allow-insecure-tls-fallback-tip", "По умолчанию VeraDesk проверяет сертификат сервера на наличие протоколов, использующих TLS.\nЕсли эта функция включена, VeraDesk пропустит данный этап и продолжит работу в случае неудачной проверки."),
        ("Disable UDP", "Отключить UDP"),
        ("disable-udp-tip", "Определяет, следует ли использовать только TCP.\nЕсли включено, VeraDesk не будет использовать UDP 21116, вместо него будет использоваться TCP 21116."),
        ("server-oss-not-support-tip", "ПРИМЕЧАНИЕ: в OSS-сервере VeraDesk эта функция отсутствует."),
        ...
        ("VeraDesk cannot reach the desktop session on the remote device, check that a desktop session is running and that VeraDesk can use it", "VeraDesk не может получить доступ к сеансу рабочего стола на удалённом устройстве, проверьте, что сеанс запущен и доступен VeraDesk"),
        ...
        ("VeraDesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old", "VeraDesk не смог получить пригодный экран от XDG Desktop Portal, библиотека PipeWire может быть слишком старой"),
        ("VeraDesk could not load a GStreamer component needed for screen capture ({})", "VeraDesk не удалось загрузить компонент GStreamer, необходимый для захвата экрана ({})"),
