...
impl ParsedPeerInfo {
    fn is_support_virtual_display(&self) -> bool {
        self.is_installed
            ...
            && (self.idd_impl == "veradesk_idd" || self.idd_impl == "amyuni_idd")
    }
}
        ...
        .await
        {
            Ok(((mut peer, direct, pk, kcp, stream_type), (feedback, rendezvous_server))) => {
                ...
                let mut status_timer =
                    crate::veradesk_interval(time::interval(Duration::new(1, 0)));
                    ...
                    tokio::select! {
                        ...
                        _ = self.timer.tick() => {
                            ...
                            if !self.read_jobs.is_empty() {
                                ...
                                self.timer = crate::veradesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                            }
                        }
                        _ = status_timer.tick() => {
                            if self.handler.is_restarting_remote_device()
                ...
                if is_remote {
                    ...
                    match fs::TransferJob::new_read(
                        ...
                        Ok(job) => {
                            ...
                            self.timer = crate::veradesk_interval(time::interval(MILLI1));
                            allow_err!(
    ...
    fn check_view_camera_support(&self, peer_version: &str, peer_platform: &str) -> bool {
            ...
            && (peer_platform == "Windows" || peer_platform == "Linux")
        {
            self.handler.msgbox(
                ...
                "upgrade_remote_veradesk_client_to_{1.3.9}_tip",
                "",
            );
        } else {
