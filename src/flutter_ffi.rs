        ...
        if key.starts_with("download-data-") {
            ...
            #[cfg(target_os = "macos")]
            {
                return if cfg!(target_arch = "x86_64") {
                    format!("veradesk-{_version}-x86_64.dmg")
                } else if cfg!(target_arch = "aarch64") {
                    format!("veradesk-{_version}-aarch64.dmg")
                } else {
