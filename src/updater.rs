...
fn acquire_mac_update_lock() -> ResultType<MacUpdateLock> {
    let path = std::path::PathBuf::from("/var/run/veradesk-update.lock");
...
fn check_update(manually: bool) -> ResultType<()> {
    ...
    if update_url.is_empty() {
        ...
        let download_url = if cfg!(feature = "flutter") {
            ...
            format!("{}/veradesk-{}-x86-sciter.exe", download_url, version)
        };
...
pub fn get_update_download_file_from_url(url: &str) -> Option<PathBuf> {
    ...
    let (allowed_owner, allowed_repo) = hbb_common::config::VERADESK_GITHUB_REPO
        .split_once('/')
        .unwrap_or(("", ""));
...
fn wait_for_failed_update_retry() {
    const FAILURE_MARKER: &str = "/var/root/.veradeskupdate_failed";
...
pub fn start_auto_update_macos() {
    let spawn_result = std::thread::Builder::new()
        .name("veradesk-auto-update".to_owned())
        .spawn(|| {
...
pub fn check_update_as_root() -> ResultType<bool> {
    ...
    if let Ok(entries) = std::fs::read_dir("/tmp") {
        for entry in entries.flatten() {
            ...
            if name_str.starts_with(".veradeskupdate-root-")
                || name_str.starts_with(".veradeskdownload-")
            {
                let path = entry.path();
    ...
    let dmg_url = format!("{}/veradesk-{}-{}.dmg", download_url, version, arch);
    ...
    let private_tmp_output = std::process::Command::new("/usr/bin/mktemp")
        .args(["-d", "/tmp/.veradeskdownload-XXXXXX"])
        .output()?;
    ...
    let filename = dmg_url.split('/').last().unwrap_or("veradesk.dmg");
...
mod tests {
    ...
    fn update_download_file_accepts_expected_github_asset_urls() {
        ...
        assert_eq!(
            ...
            Some("veradesk-1.4.0-x86_64.dmg")
        );
    }
