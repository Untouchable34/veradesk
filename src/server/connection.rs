...
pub struct Connection {
    ...
    timer: crate::VeraDeskInterval,
    file_timer: crate::VeraDeskInterval,
    ...
    // Diagnostics only, gated by `VERADESK_QOS_VERBOSE`: how long the shared
...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            hash,
            read_jobs: Vec::new(),
            timer: crate::veradesk_interval(time::interval(SEC30)),
            file_timer: crate::veradesk_interval(time::interval(SEC30)),
            file_transfer: None,
            view_camera: false,
            terminal: false,
        ...
        let mut test_delay_timer =
            crate::veradesk_interval(time::interval_at(Instant::now(), TEST_DELAY_TIMEOUT));
        ...
        let mut second_timer = crate::veradesk_interval(time::interval(Duration::from_secs(1)));
            ...
            tokio::select! {
                ...
                _ = conn.file_timer.tick() => {
                    if !conn.read_jobs.is_empty() {
                        ...
                        conn.file_timer = crate::veradesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                    }
                }
        ...
        if let Some(p) = self.start_cm_ipc_para.take() {
            tokio::spawn(async move {
                ...
                if let Err(err) = start_ipc(p.rx_to_cm, p.tx_from_cm).await {
                    ...
                    // https://github.com/rustdesk/veradesk-server-pro/discussions/382#discussioncomment-10525725, cm may start failed
                    #[cfg(windows)]
                    if !crate::platform::is_prelogin()
    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            // https://github.com/rustdesk/veradesk-server-pro/discussions/646
            ...
            match msg.union {
                ...
                Some(message::Union::KeyEvent(me)) => {
                    ...
                    if self.peer_keyboard_enabled() {
                        ...
                        // https://github.com/rustdesk/veradesk/issues/8633
                        MOUSE_MOVE_TIME.store(get_time(), Ordering::SeqCst);
                        ...
                        // https://github.com/rustdesk/veradesk/issues/6793
                    ...
                    if handle_fa {
                        ...
                        match fa.union {
                            ...
                            Some(file_action::Union::Receive(r)) => {
                                ...
                                // whenever got send/recv request, check peer version to ensure old version of veradesk
        ...
        if self.keyboard {
            if let Ok(displays) = display_service::try_get_displays() {
                ...
                if let Some(display) = displays.get(display_idx) {
                    ...
                    #[cfg(windows)]
                    if let Some(_ok) =
                        virtual_display_manager::veradesk_idd::change_resolution_if_is_virtual_display(
                            &name,
                            r.width as _,
                            r.height as _,
                        )
    ...
    async fn process_new_read_job(&mut self, mut job: fs::TransferJob, path: String) {
        ...
        self.file_timer = crate::veradesk_interval(time::interval(MILLI1));
    ...
    async fn send_printer_request(&mut self, data: Vec<u8>) {
        ...
        let path = format!("VeraDesk://FsJob//Printer/{}", get_time());
