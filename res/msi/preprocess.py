...
g_arpsystemcomponent = {
    ...
    "Contact": {
        "msi": "ARPCONTACT",
        "v": "https://github.com/veranilsoft/veradesk",
    },
    "HelpLink": {
        "msi": "ARPHELPLINK",
        "v": "https://github.com/veranilsoft/veradesk/issues/",
    },
    "ReadMe": {
        "msi": "ARPREADME",
        "v": "https://github.com/veranilsoft/veradesk",
...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="VeraDesk", help="The app name."
    )
    parser.add_argument(
...
# The app executable is handled separately: it has its own component in VeraDesk.wxs.
#
...
def gen_auto_component(app_name, dist_dir, template=False):
    return gen_content_between_tags(
        "Package/Components/VeraDesk.wxs",
        "<!--$AutoComonentStart$-->",
        "<!--$AutoComponentEnd$-->",
...
def put_app_exe_on_media2():
    ...
    target = Path(sys.argv[0]).parent.joinpath("Package/Components/VeraDesk.wxs")
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("VeraDesk", app_name)
...
def update_license_file(app_name):
    if app_name == "VeraDesk":
        return
    license_file = Path(sys.argv[0]).parent.joinpath("Package/License.rtf")
    with open(license_file, "r", encoding="utf-8") as f:
        license_content = f.read()
    license_content = license_content.replace("website veranilsoft.com and other ", "")
    license_content = license_content.replace("VeraDesk", app_name)
    license_content = re.sub(r"Veranilsoft(?: Tech Pte\.)? Ltd", app_name, license_content, flags=re.IGNORECASE)
