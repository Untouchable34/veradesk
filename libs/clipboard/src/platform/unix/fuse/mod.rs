...
pub fn init_fuse_context(is_client: bool) -> Result<(), CliprdrError> {
    ...
    let mnt_opts = [
        MountOption::FSName("veradesk-cliprdr-fs".to_string()),
        MountOption::NoAtime,
        MountOption::RO,
    ];
...
mod tests {
    ...
    fn classifies_mount_point_state_from_metadata_and_mountinfo() {
        let mount_point = std::env::temp_dir().join(format!(
            "veradesk-fuse-mount-state-test-{}-{}",
            std::process::id(),
            line!()
        ));
        let mountinfo = format!(
            "123 1 0:45 / {} rw,nosuid,nodev - fuse.veradesk veradesk rw\n",
            mount_point.display()
        );
