...
1. Use a temporary file `/tmp/veradesk_<uuid>` as a placeholder in the pasteboard.
...
2. Create temp file in `/tmp/.veradesk_*`.

### `paste_observer.rs`

Use `fsevent` to observe the paste operation with the source file `/tmp/.veradesk_*`.
