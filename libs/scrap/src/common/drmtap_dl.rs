...
impl DrmtapLib {
    fn load() -> Option<Self> {
        // Absolute path FIRST: the deb bundles the .so privately under /usr/lib/veradesk and does NOT register that dir with ld.so.
        const INSTALLED: &str = "/usr/lib/veradesk/libdrmtap.so.0";
