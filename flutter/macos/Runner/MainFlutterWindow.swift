...
class MainFlutterWindow: NSWindow {
    ...
    override func awakeFromNib() {
        ...
        let registrar = flutterViewController.registrar(forPlugin: "VeraDeskPlugin")
        ...
        FlutterMultiWindowPlugin.setOnWindowCreatedCallback { controller in
            ...
            self.setMethodHandler(registrar: controller.registrar(forPlugin: "VeraDeskPlugin"))
    ...
    private func enableNativeRelativeMouseMode(channel: FlutterMethodChannel) -> Bool {
        ...
        if result != CGError.success {
            NSLog("[VeraDesk] Failed to dissociate mouse from cursor position: %d", result.rawValue)
            return false
        }
        ...
        if state.eventMonitor == nil {
            NSLog("[VeraDesk] Failed to create event monitor for relative mouse mode")
    ...
    private func disableNativeRelativeMouseMode() {
        ...
        if result != CGError.success {
            NSLog("[VeraDesk] Failed to re-associate mouse with cursor position: %d, scheduling retry...", result.rawValue)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                ...
                if retryResult != CGError.success {
                    NSLog("[VeraDesk] Retry failed to re-associate mouse: %d. Cursor may remain locked.", retryResult.rawValue)
                }
            }
        }
    }
    ...
    public func setMethodHandler(registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "org.veradesk.veradesk/host", binaryMessenger: registrar.messenger)
