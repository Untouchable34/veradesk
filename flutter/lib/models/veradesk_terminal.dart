...
class VeraDeskTerminal extends Terminal {
  VeraDeskTerminal({
    super.maxLines,
  ...
  void _handlePrivateOsc(String code, List<String> args) {
    ...
    if (args.length != 2 || !_osc52Selection.hasMatch(args.first)) {
      debugPrint('[VeraDeskTerminal] Rejected malformed OSC 52 command');
      return;
    }
    if (args.last == '?') {
      debugPrint('[VeraDeskTerminal] Rejected OSC 52 clipboard query');
      return;
    }
    final permission = _clipboardWritePermission();
    if (permission == TerminalClipboardWritePermission.denied) {
      debugPrint('[VeraDeskTerminal] Rejected unauthorized OSC 52 write');
      return;
    }
    final selection = args.first;
    if (selection.isNotEmpty &&
        !selection.contains(_systemClipboardSelection)) {
      debugPrint('[VeraDeskTerminal] Ignored unsupported OSC 52 selection');
      return;
    }
    if (selection.replaceAll(_systemClipboardSelection, '').isNotEmpty) {
      debugPrint('[VeraDeskTerminal] Ignored unsupported OSC 52 selections');
    }
    final text = _decodeClipboardPayload(args.last);
    if (text == null) return;
    if (permission == TerminalClipboardWritePermission.unconfigured) {
      debugPrint('[VeraDeskTerminal] Blocked OSC 52 write pending consent');
  ...
  Future<void> _writeClipboard(String text) async {
    ...
    debugPrint(
        '[VeraDeskTerminal] OSC 52 clipboard write requires interaction');
    _onClipboardWriteBlocked?.call(text);
  }

  String? _decodeClipboardPayload(String payload) {
    if (payload.length > _maxBase64EncodedLength(_maxClipboardWriteBytes)) {
      debugPrint('[VeraDeskTerminal] Rejected oversized OSC 52 payload');
      return null;
    }
    try {
      ...
      debugPrint('[VeraDeskTerminal] Rejected malformed OSC 52 payload');
      return null;
    }
  }
