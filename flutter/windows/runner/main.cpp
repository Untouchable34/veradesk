...
typedef char** (*FUNC_VERADESK_CORE_MAIN)(int*);
typedef void (*FUNC_VERADESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_VERADESK_GET_APP_NAME)(wchar_t*, int);
typedef int (*FUNC_VERADESK_IS_DISABLE_INSTALLATION)();
...
{
  HINSTANCE hInstance = LoadLibraryA("libveradesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libveradesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_VERADESK_CORE_MAIN veradesk_core_main =
      (FUNC_VERADESK_CORE_MAIN)GetProcAddress(hInstance, "veradesk_core_main_args");
  if (!veradesk_core_main)
  {
    std::cout << "Failed to get veradesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_VERADESK_FREE_ARGS free_c_args =
      (FUNC_VERADESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = veradesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "VeraDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);
  FUNC_VERADESK_IS_DISABLE_INSTALLATION veradesk_is_disable_installation =
      (FUNC_VERADESK_IS_DISABLE_INSTALLATION)GetProcAddress(hInstance, "veradesk_is_disable_installation");
  bool is_disable_installation =
      veradesk_is_disable_installation && veradesk_is_disable_installation() != 0;
  ...
  std::wstring app_name = L"VeraDesk";
  FUNC_VERADESK_GET_APP_NAME get_veradesk_app_name = (FUNC_VERADESK_GET_APP_NAME)GetProcAddress(hInstance, "get_veradesk_app_name");
  if (get_veradesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_veradesk_app_name(app_name_buffer, 512) == 0) {
