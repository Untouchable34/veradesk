...
#
# Script to build F-Droid release of VeraDesk
#
# Copyright (C) 2024, The VeraDesk Authors
...
#
# - versionName from https://github.com/rustdesk/veradesk/releases/download/fdroid-version/veradesk-version.txt
# - versionCode from https://github.com/rustdesk/veradesk/releases/download/fdroid-version/veradesk-version.txt
...
arm64-v8a)
	...
	VERADESK_FEATURES='flutter,hwcodec'
	;;
armeabi-v7a)
...
prebuild)
	...
	# Flutter used to compile main Veradesk library
	...
	# Install Flutter version for VeraDesk library build
...
build)
	...
	cargo ndk \
		...
		--locked \
		--release \
		--features "${VERADESK_FEATURES}"

	mkdir -p "flutter/android/app/src/main/jniLibs/${ANDROID_ABI}"

	cp "target/${RUST_TARGET}/release/liblibveradesk.so" \
		"flutter/android/app/src/main/jniLibs/${ANDROID_ABI}/libveradesk.so"
