...
hbb_name = 'veradesk' + ('.exe' if windows else '')
...
def get_features(args):
    ...
    if args.drm:
        ...
        # package NAMED `veradesk` carrying the consent-bypass backend and the root-side uinput
...
def generate_control_file(version):
    ...
    content = """Package: veradesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: veradesk <info@veranilsoft.com>
Homepage: https://veranilsoft.com
...
# commit — the same way veradesk sources its other native build deps (vcpkg,
# flutter_rust_bridge, ...), rather than carrying a git submodule. It is the ONLY
# pin for the drm backend: veradesk dlopens this .so at runtime and does not depend on
...
def build_libdrmtap_so():
    ...
    # the shared one explicitly (veradesk dlopens the .so and never needs the archive).
...
DRM_PACKAGE_NAME = 'veradesk-unattended-wayland'
...
def stage_libdrmtap_into_deb(so_path):
    ...
    # symlink is needed: libdrmtap is resolved by ABSOLUTE path (/usr/lib/veradesk/libdrmtap.so.0) at
    # the in-process dlopen site (drmtap_dl.rs), so the deb does NOT drop /usr/lib/veradesk into the
    ...
    system2('mkdir -p tmpdeb/usr/lib/veradesk')
    ...
    system2(f'cp "{so_path}" tmpdeb/usr/lib/veradesk/')
    system2(f'ln -sf "{so_basename}" tmpdeb/usr/lib/veradesk/libdrmtap.so.0')


def _max_glibc_minor(path):
    # Read from .dynstr rather than via objdump so packaging needs no binutils; chunked because
    # libveradesk.so is ~45 MB.
    best = 0
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/veradesk/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/veradesk/lib/libveradesk.so')
             + glob.glob('tmpdeb/usr/share/veradesk/veradesk')
...
def retarget_control_to_drm_variant():
    ...
    out = []
    for line in lines:
        if line.startswith('Package: veradesk'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append('Conflicts: veradesk\nReplaces: veradesk\nProvides: veradesk\n')
            ...
            out.append(line)
    body = ''.join(out)
    # Fail loudly rather than silently shipping a package that says `veradesk`: a stock control file
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/veradesk')
    system2('mkdir -p tmpdeb/usr/share/veradesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/veradesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/veradesk/')
    system2(
        'cp ../res/veradesk.service tmpdeb/usr/share/veradesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/veradesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/veradesk.svg')
    system2(
        'cp ../res/veradesk.desktop tmpdeb/usr/share/applications/veradesk.desktop')
    system2(
        'cp ../res/veradesk-link.desktop tmpdeb/usr/share/applications/veradesk-link.desktop')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/veradesk/files/polkit && chmod a+x tmpdeb/usr/share/veradesk/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb veradesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('veradesk.deb', '../veradesk-%s.deb' % version)
    if ships_so:
        # Named apart from the stock package so installing the consent-free variant is a deliberate act.
        os.rename('../veradesk-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")


DRMTAP_DLOPEN_MARKER = b'/usr/lib/veradesk/libdrmtap.so.0'
...
def _carries_drmtap_marker(path, marker=DRMTAP_DLOPEN_MARKER):
    ...
    # libveradesk.so is ~45 MB and there is no reason to hold it all in memory, and the `with`
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/veradesk/lib/libveradesk.so')
                + glob.glob('tmpdeb/usr/share/veradesk/veradesk') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    system2('rm tmpdeb/usr/bin/veradesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/veradesk/')
    system2(
        'cp ../res/veradesk.service tmpdeb/usr/share/veradesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/veradesk.png')
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/veradesk/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/veradesk/libdrmtap.so tmpdeb/usr/share/veradesk/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibveradesk.dylib target/release/libveradesk.dylib")
    os.chdir('flutter')
    ...
    os.rename("veradesk.dmg", f"../veradesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libveradesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --locked --features {features} --lib --release')
        if not os.path.exists("target/release/libveradesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/veradesk.exe')
    os.chdir('../..')
    if os.path.exists('./veradesk_portable.exe'):
        os.replace('./target/release/veradesk-portable-packer.exe',
                   './veradesk_portable.exe')
    else:
        os.rename('./target/release/veradesk-portable-packer.exe',
                  './veradesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/veradesk_portable.exe')
    os.rename('./veradesk_portable.exe', f'./veradesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/veradesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/veradesk.exe')
        system2('mv target/release/veradesk.exe target/release/VeraDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\veradesk.exe')
        else:
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        system2(
            f'cp -rf target/release/VeraDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/veradesk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/veradesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/veradesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv veradesk-%s-0-x86_64.pkg.tar.zst veradesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./veradesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/veradesk-%s-0.x86_64.rpm ./veradesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall veradesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/veradesk-%s-0.x86_64.rpm ./veradesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall veradesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/veradesk*.deb ./flutter/veradesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/VeraDesk.app/Contents/MacOS/veradesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/VeraDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/veradesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/VeraDesk.app/Contents/MacOS/veradesk
    #rcodesign sign --p12-file ~/.p12/veradesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/VeraDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/veradesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/VeraDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/VeraDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/VeraDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "VeraDesk %s.dmg" "target/release/bundle/osx/VeraDesk.app"' % version)
                os.rename('VeraDesk %s.dmg' %
                          version, 'veradesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/veradesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./veradesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./veradesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple veradesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/VeraDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/veradesk*.deb ./veradesk.deb')
                system2('dpkg-deb -R veradesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/veradesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2('strip tmpdeb/usr/bin/veradesk')
                system2('mkdir -p tmpdeb/usr/share/veradesk')
                system2('mv tmpdeb/usr/bin/veradesk tmpdeb/usr/share/veradesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/veradesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb veradesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('veradesk.deb', 'veradesk-%s.deb' % version)
